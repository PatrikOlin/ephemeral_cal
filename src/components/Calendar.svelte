    ...
    {#each { length: 6 } as week, idxw (idxw)}
        {#if current[idxw]}
            {#each { length: 7 } as day, idxd (idxd)}
                ...
                {:else if (idxw < 1)}
                    <span class="date other"  on:click={setAvailable(current[idxw][idxd])}>{ prev[prev.length - 1][idxd] }</span>
                {:else}
                    <span class="date other" on:click={setAvailable(current[idxw][idxd])}>{ next[0][idxd] }</span>
                {/if}
            {/each}
        {/if}
    {/each}
